...
#define OUTCODE(x, y) (((x) < xmin) ? 1 : (((x) > xmax) ? 2 : 0)) + (((y) < ymin) ? 4 : (((y) > ymax) ? 8 : 0))
	...
	void Surface::transparent_copy_to(Surface* a_Dst, int a_X, int a_Y)
	{
		Pixel* dst = a_Dst->get_buffer();
		Pixel* src = m_Buffer;
		if ((src) && (dst))
		{
			...
			if ((srcwidth > 0) && (srcheight > 0))
			{
				...
				for (int y = 0; y < srcheight; y++)
				{
					for (int x = 0; x < srcwidth; x++) dst[x] = add_over(dst[x], src[x]);
					dst += dstpitch;
					src += srcpitch;
				}
			}
	...
	void Surface::draw_over(Surface* a_Dst, int a_X, int a_Y)
	{
		Pixel* dst = a_Dst->get_buffer();
		Pixel* src = m_Buffer;
		if ((src) && (dst))
